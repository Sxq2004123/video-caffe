...
bool Blob<Dtype>::ShapeEquals(const BlobProto& other) {
  int forgotten_dim_size = 1;
  if (other.has_num() && other.has_channels() && other.has_height() &&
      other.has_width() && other.data_size() > 0) {
    forgotten_dim_size = other.data_size() / (other.num() * other.channels() *
                         other.height() * other.width());
  }
  const bool is_legacy_C3D = forgotten_dim_size > 1;
  if (is_legacy_C3D) {
    LOG(INFO) << "Legacy C3D blob detected.";
  }
...
void Blob<Dtype>::FromProto(const BlobProto& proto, bool reshape) {
  ...
  if (proto.has_num() && proto.has_channels() && proto.has_height() &&
      proto.has_width() && proto.diff_size() > 0) {
    forgotten_dim_size = proto.diff_size() / (proto.num() * proto.channels() *
  ...
  if (reshape) {
    ...
    if ((proto.has_num() || proto.has_channels() ||
        proto.has_height() || proto.has_width()) && !is_legacy_C3D) {
      // Using deprecated 4D Blob dimensions --
      // shape is (num, channels, height, width).
      shape.resize(4);
      shape[0] = proto.num();
  ...
  } else if (proto.diff_size() > 0 && !is_legacy_C3D) {
