...
bool Blob<Dtype>::ShapeEquals(const BlobProto& other) {
  const int forgotten_dim_size = other.data_size() / (other.num() *
    other.channels() * other.height() * other.width());
  const bool is_legacy_C3D_other = forgotten_dim_size > 1;
  if (is_legacy_C3D_other) {
  ...
  } else if ((other.has_num() || other.has_channels() ||
      other.has_height() || other.has_width()) &&
      is_legacy_C3D_other) {
...
void Blob<Dtype>::FromProto(const BlobProto& proto, bool reshape) {
  ...
  const bool is_legacy_C3D_proto = forgotten_dim_size > 1;
  ...
  if (reshape) {
    ...
    if ((proto.has_num() || proto.has_channels() ||
        proto.has_height() || proto.has_width()) && !is_legacy_C3D_proto) {
      // Using deprecated 4D Blob dimensions --
      // shape is (num, channels, height, width).
      shape.resize(4);
      shape[0] = proto.num();
